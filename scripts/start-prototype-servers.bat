...
start "Omni" cmd /k "cd /d "%~dp0..\projects\prototypes\rights-management\omni" && npm run dev -- --port 3001 --host"
timeout /t 3 /nobreak >nul

echo 启动云仓系统 Depot (端口 3002)...
start "Depot" cmd /k "cd /d "%~dp0..\projects\prototypes\rights-management\depot" && npm run dev -- --port 3002 --host"
timeout /t 3 /nobreak >nul

echo 启动灵霄系统 Cel (端口 3003)...
start "Cel" cmd /k "cd /d "%~dp0..\projects\prototypes\rights-management\cel\frontend" && npm run dev -- --port 3003 --host"
timeout /t 3 /nobreak >nul
