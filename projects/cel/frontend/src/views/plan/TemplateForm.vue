...
const rules = {
  ...
  planId: [{ required: true, message: '请输入策划ID', trigger: 'blur' }]
}
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put('/api/template', formData)
      ElMessage.success('编辑成功')
    } else {
      await request.post('/api/template', formData)
...
onMounted(async () => {
  ...
  if (id) {
    isEdit.value = true
    const res = await request.get(`/api/template/${id}`)
